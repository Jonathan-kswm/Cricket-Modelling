...
cts_condition <- function(match, event, parameter, modifyer, inns) {
  ...
  #with switch we are going to set up seeing if for a team in a game with a condition what happened to that team
  switch(event,
         ...
         "Sixes" = if (param_codes(sum(index$runs_batter == 6), parameter) == TRUE){
                    as.character(index$batting_team[1]) #batting team is constant
                    }else {return(NULL)},
         "Runs" = if (param_codes(tail(index$cumulative_runs, n=1), parameter) == TRUE){
                    as.character(index$batting_team[1])
                  }else {return(NULL)},
         "Wickets" = if (param_codes(tail(index$cumulative_wickets, n=1), parameter) == TRUE){
                      as.character(index$bowling_team[1])
                      }else {return(NULL)}
          )
}
...
conditional <- function(event, parameter = NULL, modifier = NULL, inns = NULL, data = wt20_data) {
  #catch errors in input
  if (event %in% valid_inputs$Binary_inputs || event %in% valid_inputs$Cts_inputs){
  } else{cat("Error: Invalid Input\n ,event, is not a valid Event (See Events)"); return(invisible(NULL))}
  if (parameter %in% valid_inputs$Parameter || is.null(parameter)){
  } else{cat("Error: Invalid Input\n, parameter, is not a valid Parameter (See Parameters)"); return(invisible(NULL))}
  if (inns %in% valid_inputs$inns || is.null(inns)){
  } else{cat("Error: Invalid Input\n, inns, is not a valid Innings (See Innings)"); return(invisible(NULL))}
...
#print statement:
  ...
  cat("Condition:", event, "\n",
      ...
      "Outcomes Tied:", ties, "\n\n",
      "Win Rate of Condition:", if (total > 0) round(wins / total, 3) else NA, "\n")
}
